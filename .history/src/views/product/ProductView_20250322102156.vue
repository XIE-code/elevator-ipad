...
onMounted(() => {
    // 获取所有导航项
    const nav = document.querySelectorAll(".web .web-left .navigation");
    console.log(nav);

    // 移除所有导航项的高亮样式
    function removeActiveClasses() {
        nav.forEach((item) => {
            item.classList.remove(".web .web-left .navigation .active");
        });
    }
    ...
    nav.forEach((item) => {
        item.addEventListener("click", () => {
            removeActiveClasses(); // 移除所有导航项的高亮样式
            item.classList.add(" .web .web-left .navigation .active"); // 为当前点击的导航项添加高亮样式
        });
    });
});
</script>
...
/* 添加高亮效果的样式 */
.web .web-left .navigation .active {
    background: red; /* 高亮时的背景颜色 */
}
</style>
