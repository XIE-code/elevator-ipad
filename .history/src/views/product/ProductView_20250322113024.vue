...
function setActive(index) {
    ...
    // 为当前点击的导航项添加高亮样式
    navigationItems.value[index].classList.add('active');
}

onMounted(() => {
    // 获取所有导航项
    navigationItems.value = document.querySelectorAll('.web-left .navigation');

    // 为每个导航项添加点击事件监听器
    navigationItems.value.forEach(item => {
        item.addEventListener('click', () => {
            setActive(Array.from(navigationItems.value).indexOf(item));
        });
    });
});
</script>
