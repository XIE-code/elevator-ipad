...
function handleNavigationClick(event) {
    ...
    // 移除所有导航项的高亮样式
    navigationItems.value.forEach((item) => item.classList.remove("active"));

    // 为当前点击的导航项添加高亮样式
    target.classList.add("active");

    // 检查是否是别墅电梯的导航项
    if (target.textContent.includes("别墅电梯")) {
...
onMounted(() => {
    // 获取所有导航项
    navigationItems.value = document.querySelectorAll(
        ".web .web-left .navigation"
    );
...
/* 别墅电梯的高亮效果 */
.navigation.highlight-red {
    transform: translateX(10px); /* 平移效果 */
}
</style>
